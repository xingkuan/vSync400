...
/* JLEE, 07/17    
   public int getRequest() {
      // if there are requests, get the first one and delete... otherwise, return -1
      // also deletes any same command for the same table
      int rtv=-1;
      Connection lrepConn;
      Statement lrepStmt;
      ResultSet lrRset;
      ...
      try {
         lrepConn = DriverManager.getConnection(srcCred.getURL(), srcCred.getUser(), srcCred.getPWD());
         lrepConn.setAutoCommit(false);
         lrepStmt = lrepConn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
         //lrRset = lrepStmt.executeQuery("select cmd_id, table_id, request, request_id from " + OVSRequest + " where table_id=" + tableID + " order by cmd_id ");getCmdQueue
         lrRset = lrepStmt.executeQuery("select cmd_id, table_id, request, request_id from " + srcCred.getCmdQueue() + " where table_id=" + tableID + " order by request_id ");
         if (lrRset.next()) {
            rtv=lrRset.getInt("request");
            lRequest=lrRset.getInt("request");
            OVSrequest reqData = new OVSrequest(lrRset.getTimestamp("cmd_id"), lrRset.getInt("table_id"), lRequest, lrRset.getLong("request_id")) ;
            lrRset.deleteRow();
            lrepConn.commit();
            ovLogger.log(label + " audit request received for table: " + rtv);
         } else { 
            rtv=-1;
         } 
         lrRset.close();
         // if there was a request, delete all same requests for this table
         if (lRequest != -1) {
            lrepStmt.executeUpdate("delete from " + srcCred.getCmdQueue() + " where table_id= " + tableID + " and request = " + lRequest );
            lrepConn.commit();
            lrepStmt.close();
            lrepConn.close();
            ...
            lrepConn.setAutoCommit(false);
            lrepStmt = lrepConn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
            lrepStmt.executeUpdate("delete from " + repCred.getCmdQueue() + " where object_id= " + tableID + " and request = " + lRequest + " and cmd_type=2" );
            lrepConn.commit();
         }
         lrepStmt.close();
         lrepConn.close();
      } catch(SQLException e) {
         ovLogger.log(e.getMessage());
      }
      return rtv;

   }
   public void putAuditRequest() {
      // creates an audit request entry into table's cmd queue
      Connection lrepConn;
      Statement lrepStmt;
     // ovLogger.log(label + "putAuditRequest");
      try {
         //ovLogger.log(label + "putAuditRequest opening connection");
         //ovLogger.log(label + "creds " + repCred.getURL() + repCred.getUser() + repCred.getPWD());
         lrepConn = DriverManager.getConnection(repCred.getURL(), repCred.getUser(), repCred.getPWD());
         //lrepConn.setAutoCommit(false);
         //ovLogger.log(label + "creating putAudit statement");
         lrepStmt = lrepConn.createStatement(ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE);
         //ovLogger.log(label + "executing putAudit insert");
         lrepStmt.executeUpdate(" insert into " + repCred.getCmdQueue()  + " (object_id, request, cmd_type) values (" + tableID + ",4, 2)" );
         //ovLogger.log(label + "commiting putAudit statement");
         lrepConn.commit();
         lrepStmt.close();
         lrepConn.close();
         //ovLogger.log(label + "closing putAudit statement");
      } catch(SQLException e) {
         ovLogger.log(label + e.getMessage());
      }
   }
   public OVSrequest getRequestData() {
      return reqData;
   }
*/
